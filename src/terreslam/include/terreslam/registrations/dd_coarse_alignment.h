...
namespace terreslam
{
	...
	float fit3DofRANSAC(const std::vector<cv::Point3f>& src_3d, const std::vector<cv::Point3f>& dst_3d, float* best_param,  bool* inliers, const cv::Point2f center, const float inlierMaxEr, const int niter, const bool debug);
}
