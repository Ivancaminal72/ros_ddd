...
//			time_stamp=T.time_stamp;
		...
		Transform(cv::Mat rvec, cv::Mat tvec)
		{
			cv::Mat Rot;
		...
		Transform(Eigen::Matrix4f trans)
		{
			for(int i=0;i<3;i++)
			{
				t(i)=double(trans(i,3));
				for(int j=0;j<3;j++)
				{
					R(i,j)=double(trans(i,j));
				}
			}
		}
...
//		double time_stamp;
		...
		friend std::ostream & operator << (std::ostream &os, const terreslam::Transform &T)
		{
		...
		static Eigen::Matrix3d exp_SO3(Eigen::Vector3d phi)
		{
			double n=phi.norm();
			Eigen::Vector3d a=phi.normalized();
			if(n<0.0001) a.setZero();
			return cos(n)*Eigen::Matrix3d::Identity()
				   +(1-cos(n))*a*a.transpose()
				   +sin(n)*skew_sym(a);
		}

		static Transform exp_SE3(Eigen::Vector3d rho, Eigen::Vector3d phi)
		{
			Eigen::Matrix3d C=exp_SO3(phi);
			double n=phi.norm();
			Eigen::Vector3d a=phi.normalized();
			Eigen::Matrix3d J;
			if(n<0.0001) { J=Eigen::Matrix3d::Identity(); }
			else 
			{
				J=Eigen::Matrix3d::Identity()*sin(n)/n 
				 +(1-sin(n)/n)*a*a.transpose()
				 +skew_sym(a)*(1-cos(n))/n;
			}
			return Transform(C,J*rho);
		}
...
// 		Eigen::Matrix3d cov, info, sqrt_info;
...
// 		EdgePoint* pair_point;
// 		double weight;
// 		// lambda_pk_l=[lambda_pk_1,...,lambda_pk_6]^T;
// 		Vector6d lambda_pk_l;
...
// 	struct KeyPoint 
// 	{
// 		KeyPoint(cv::Point2d pxc, cv::Point2d pxr, cv::Point3d ptc, cv::Point3d ptr)
// 		{
// 			pixel_cur(0)=pxc.x; pixel_cur(1)=pxc.y;
// 			pixel_ref(0)=pxr.x; pixel_ref(1)=pxr.y;
// 			point_cur(0)=ptc.x; point_cur(1)=ptc.y; point_cur(2)=ptc.z;
// 			point_ref(0)=ptr.x; point_ref(1)=ptr.y; point_ref(2)=ptr.z;
// 		}

// 		KeyPoint(double xc, double yc, double zc,
// 				 double xr, double yr, double zr)
// 		{
// 			point_cur(0)=xc; point_cur(1)=yc; point_cur(2)=zc;
// 			point_ref(0)=xr; point_ref(1)=yr; point_ref(2)=zr;
// 		}

// 		Eigen::Vector2d pixel_cur,pixel_ref;
