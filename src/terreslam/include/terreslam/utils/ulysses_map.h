 ...
 *    Last Modified: 2021-02-12 13:30:30
 */
...
#include <terreslam/utils/ulysses_types.h>
...
//		void insert(Feature *cur, Feature *ref, Landmark *lm);
		void insert(Feature *cur, Feature *ref, Scan *scan);
		void insert(Feature *cur, Landmark *lm, Scan *scan, Map *map);
...
//	};
	...
	public:
		...
		iterLandmark eraseLandmark(std::string id);
...
//		void saveTimes(const std::string &folder) const;
		void vis(boost::shared_ptr<pcl::visualization::PCLVisualizer> v, bool fuse=true);
...
//		void saveTrajVO(const std::string &filename) const;
		...
		void pushPoints(Landmark *lm, ptrPointCloud points, bool fuse=true);
	};
...
//		void savePRCMap(const std::string &folder, LandmarkAssociation *fa);
		...
		void visPlaneLines(boost::shared_ptr<pcl::visualization::PCLVisualizer> v);
		void visEdges(boost::shared_ptr<pcl::visualization::PCLVisualizer> v);
		void visKeypoints(boost::shared_ptr<pcl::visualization::PCLVisualizer> v);
		...
		void addKeyPoint(KeyPoint *kp) {key_points.push_back(kp);}
		int sizeKeyPoint() const {return key_points.size();}
		KeyPoint* keyPoint(int i) const {return key_points[i];}
